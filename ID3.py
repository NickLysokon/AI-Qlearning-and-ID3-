...
class ID3:
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        info_gain_value = current_uncertainty - p * left_entropy - (1 - p) * right_entropy
        # ========================
    ...
    def build_tree(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        # Base case: if there are no more rows or all the labels are the same, return a leaf node
        if len(rows) == 0 or len(set(labels)) == 1:
            return Leaf(rows,labels)

        #pruning of decison tree
        if len(rows)<= self.min_for_pruning:
            return Leaf(rows,labels)

        # Find the best question to ask at this point
